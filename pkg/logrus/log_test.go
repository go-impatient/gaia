...
func checkErrorf(t *testing.T, log Log, buf *bytes.Buffer) {
...
func TestConcurrentMods(t *testing.T) {
	...
	for i := 0; i < count; i++ {
		...
		go func(i int) {
			...
			log.WithError(errors.New("No alert matched in alert config.")).WithField("key"+iStr, "value"+iStr).Debugf("Run %d", i)
		}(i)
	}
	wg.Wait()
	f.Seek(0, 0)
...
func customLogger(output io.Writer) Log {
	log := New()
